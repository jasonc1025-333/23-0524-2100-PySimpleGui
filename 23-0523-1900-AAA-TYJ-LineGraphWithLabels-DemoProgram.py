...
while True:
    ...
    for i in range(30):
        ...
        if i > 0:
            ...
            graph.DrawLine(((i-1) * BAR_SPACING + EDGE_OFFSET+ BAR_WIDTH/2 ,  before_value+10)  ,  (i * BAR_SPACING + EDGE_OFFSET+ BAR_WIDTH/2 , graph_value+10 ),color='green', width=10 )

        graph.DrawText(text=graph_value, location=(i*BAR_SPACING+EDGE_OFFSET+2, graph_value+10))

        ###jwc o graph.DrawPoint((i * BAR_SPACING + EDGE_OFFSET+ BAR_WIDTH/2 ,graph_value), size=3 ,color='blue',)
        graph.DrawPoint((i * BAR_SPACING + EDGE_OFFSET+ BAR_WIDTH/2 ,graph_value), size=30 ,color='blue',)
